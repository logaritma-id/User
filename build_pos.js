...
function injectPos(filepath, categoryName, posHtml, posJs) {
    let content = fs.readFileSync(filepath, 'utf8');

    // 1. Inject Nav (only if not already there)
    const navLink = `
    ...
    if (!content.includes('href="#pos-section"')) {
        content = content.replace(/(<a href="#uji-dampak".*?<\/a>)/s, `$1${navLink}`);
    }

    // 2. Inject POS HTML (only if not already there)
    const posMarker = 'id="pos-section"';
    if (!content.includes(posMarker)) {
        const col3Marker = '<!-- COLUMN 3: RIGHT SIDEBAR';
        const parts = content.split(col3Marker);
        if (parts.length >= 2) {
            let leftPart = parts[0];
            const lastDivIdx = leftPart.lastIndexOf('</div>');
            if (lastDivIdx !== -1) {
                leftPart = leftPart.substring(0, lastDivIdx) + posHtml + '\n            </div>\n' + leftPart.substring(lastDivIdx + 6);
            }
            content = leftPart + col3Marker + parts.slice(1).join(col3Marker);
        }
    }

    // 3. Clean up old injected JS (which was put before window.copyLtmAiOutput)
    if (content.includes('// ===== POS')) {
        // We will remove it and re-inject it as a standalone script tag at the end of body.
        const jsStartMarker = '// ===== POS';
        const jsEndRegex = /\s*\(\)\);\s*window\.copyLtmAiOutput/;
        
        let startIdx = content.indexOf(jsStartMarker);
        if (startIdx !== -1) {
            let tempContent = content.substring(startIdx);
            let match = tempContent.match(jsEndRegex);
            if (match) {
                let endIdx = startIdx + match.index + match[0].indexOf('window.copyLtmAiOutput');
                content = content.substring(0, startIdx) + content.substring(endIdx);
            }
        }
    }

    // 4. Inject POS JS as a standalone <script> tag before </body>
    const jsScriptTag = `\n    <script>\n${posJs}\n    </script>\n`;
    if (!content.includes(jsScriptTag.trim())) {
        content = content.replace('</body>', jsScriptTag + '</body>');
    }

    fs.writeFileSync(filepath, content, 'utf8');
    console.log(`Fixed POS for ${categoryName} in ${filepath}`);
}
...
const jsKuliner = `
        ...
        window.posKuliner = (() => {
            ...
            // Auto initialize without DOMContentLoaded since script is at the bottom
            renderMenus(); 
            renderTxs(); 
            updateSummary();
...
// Execute injections
injectPos(path.join(__dirname, "tools/kuliner/index.html"), "Kuliner", htmlKuliner, jsKuliner);
injectPos(path.join(__dirname, "tools/fashion/index.html"), "Fashion", htmlFashion, jsFashion);
injectPos(path.join(__dirname, "tools/percetakan/index.html"), "Percetakan", htmlPercetakan, jsPercetakan);
injectPos(path.join(__dirname, "tools/pkl/index.html"), "PKL", htmlPkl, jsPkl);
injectPos(path.join(__dirname, "tools/distributor/index.html"), "Distributor", htmlDistributor, jsDistributor);
