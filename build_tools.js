...
<body class="bg-brand-bg text-slate-300 font-sans antialiased h-screen flex overflow-hidden selection:bg-brand-accent1/30 selection:text-white">
    ...
    <aside id="sidebar" class="fixed lg:relative inset-y-0 left-0 z-40 w-72 bg-brand-bg border-r border-brand-border transform -translate-x-full lg:translate-x-0 transition-transform duration-300 ease-in-out flex flex-col h-full pt-16 lg:pt-0">
        ...
        <div class="p-4 shrink-0 border-t border-brand-border">
            <div class="bg-brand-surface rounded-xl p-4 border border-white/5 relative overflow-hidden">
                ...
                <p id="engine-status-desc" class="text-[10px] text-slate-400">Tim Logaritma siap bekerja</p>
                <div class="mt-3">
    ...
    <main class="flex-1 h-full overflow-y-auto custom-scrollbar bg-brand-bg relative pt-16 lg:pt-0">
        ...
        <div class="hidden lg:flex h-20 items-center justify-end px-8 border-b border-brand-border sticky top-0 bg-brand-bg/80 backdrop-blur-md z-10">
            <div class="flex items-center gap-4">
                <div class="flex bg-brand-surface rounded-full p-1 border border-white/10 select-none">
                    <span class="px-3 py-1 text-xs font-bold bg-white/10 rounded-full text-white cursor-default">ID</span>
                    <span onclick="Swal.fire({icon: 'info', title: 'Coming Soon', text: 'English language support will be available soon!', background: '#0f172a', color: '#cbd5e1', confirmButtonColor: '#10b981'})" class="px-3 py-1 text-xs font-bold text-slate-500 hover:text-white transition cursor-pointer">EN</span>
                </div>
                <div onclick="window.showUserProfile()" class="w-8 h-8 rounded-full bg-gradient-to-r from-brand-accent1 to-brand-accent2 flex items-center justify-center text-white text-xs font-bold cursor-pointer hover:opacity-80 transition shadow-lg shadow-brand-accent1/20 select-none">U</div>
            </div>
        </div>
        ...
        <div class="px-6 sm:px-8 pt-4 pb-20 max-w-7xl mx-auto min-h-full">
            ...
            <div id="dasbor" class="tab-pane flex flex-col xl:flex-row gap-6">
                ...
                <div class="flex-1 space-y-6">
                    ...
                    <h1 class="text-3xl sm:text-4xl font-bold text-white font-heading">Sistem Rekayasa Operasional<br/><span id="dashboard-cat-title" class="text-transparent bg-clip-text bg-gradient-to-r from-brand-accent1 to-brand-accent2">${category.name}</span></h1>
    ...
    <div id="paywall-modal" class="fixed inset-0 z-[100] hidden items-center justify-center modal-overlay px-4">
        <div class="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-lg p-8 relative shadow-2xl overflow-hidden">
            <button id="close-paywall-btn" class="absolute top-4 right-4 text-slate-400 hover:text-white z-20">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
            </button>
            <div class="text-center relative z-10">
                <div class="w-16 h-16 bg-gradient-to-br from-amber-400 to-orange-500 rounded-full mx-auto flex items-center justify-center text-3xl shadow-lg shadow-orange-500/30 mb-4">👑</div>
                <h3 class="text-2xl font-bold text-white mb-2 font-heading">Fitur Eksklusif Premium</h3>
                <p class="text-slate-400 text-sm mb-6">Tingkatkan keanggotaan Anda untuk mengakses Kalkulator LTM, Kasir POS, dan AI Pembuat Dokumen secara tanpa batas.</p>
                
                <div class="space-y-3">
                    <a href="https://baimwarunkarsi.myr.id/m/logaritma-umkm-pro-akses-ai-profit-engine" target="_blank" class="block w-full bg-gradient-to-r from-amber-500 to-orange-600 hover:from-amber-400 hover:to-orange-500 text-white font-bold py-3.5 rounded-xl transition shadow-lg shadow-orange-500/25">
                        🌟 Langganan Baru (PRO)
                    </a>
                    <a href="https://baimwarunkarsi.myr.id/pay-membership" target="_blank" class="block w-full bg-slate-800 hover:bg-slate-700 border border-slate-600 text-white font-bold py-3.5 rounded-xl transition">
                        🔄 Perpanjang Layanan
                    </a>
                </div>
                <p class="text-xs text-slate-500 mt-6">Setelah pembayaran berhasil, Admin akan memverifikasi dan mengaktifkan fitur PRO Anda.</p>
            </div>
    ...
    <script type="module">
        ...
        const _userStr = localStorage.getItem('logarithm_current_user');
        let businessName = '';
        let ownerName = '';
        let waNumber = '';
        ...
        try { 
            if(_userStr) {
                const _userData = JSON.parse(_userStr); 
                businessName = _userData.namaBisnis || _userData.bisnis || '';
                ownerName = _userData.namaPemilik || _userData.nama || '';
                waNumber = _userData.whatsapp || _userData.wa || '';
                const st = (_userData.status || '').toUpperCase();
                if(st.includes('PRO') || st.includes('PREMIUM')) isPremiumUser = true;
            } 
        } catch(e) {}
        
        const businessContextStr = businessName ? \` bernama '\${businessName}' milik \${ownerName}\` : '';
        
        document.addEventListener('DOMContentLoaded', async () => {
            // DB Sync
            if(waNumber && window.LogaritmaDB) {
                try {
                    const dbUser = await window.LogaritmaDB.getUserByWA(waNumber);
                    if(dbUser) {
                        const status = (dbUser.status || '').toUpperCase();
                        isPremiumUser = status.includes('PREMIUM') || status.includes('PRO');
                        localStorage.setItem('logarithm_current_user', JSON.stringify(dbUser));
                    }
                } catch(e) {
            ...
            if(businessName) {
                const titleEl = document.getElementById('dashboard-cat-title');
                if(titleEl && !titleEl.innerText.includes(businessName)) titleEl.innerText += ' - ' + businessName;
                ...
                document.querySelectorAll('h2').forEach(h2 => {
                    if(h2.innerText.includes('Uji Dampak Kegiatan') && !h2.innerText.includes(businessName)) {
                        h2.innerText = h2.innerText.replace('Uji Dampak Kegiatan', 'Uji Dampak - ' + businessName);
                    } else if((h2.innerText.includes('Kasir POS') || h2.innerText.includes('Kasir PKL')) && !h2.innerText.includes(businessName)) {
                        h2.innerText = h2.innerText + ' (' + businessName + ')';
                    }
                });
            }
            
            // Paywall Modal Close Logic
...
\n          const p1 = "AIzaSy";
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // Legacy premium logic removed, handled globally above.
    ...
    window.showUserProfile = () => {
        const userStr = localStorage.getItem('logarithm_current_user');
        if(!userStr) {
            Swal.fire({
                title: 'Profil Pengguna',
                text: 'Anda sedang menggunakan alat tanpa data profil (Mode Gratis).',
                icon: 'info',
                background: '#0f172a',
                color: '#cbd5e1',
        ...
        const nama = user.namaPemilik || user.nama || 'Pengguna UMKM';
        const bisnis = user.namaBisnis || user.bisnis || '-';
        const kategori = user.kategori || '-';
        
        Swal.fire({
            title: 'Profil Anda',
            html: \`
                <div class="text-left mt-2 text-sm text-slate-300 bg-white/5 p-4 rounded-xl border border-white/10">
                    <p class="mb-2"><span class="text-slate-500 inline-block w-20">Nama</span>: <strong class="text-white">\${nama}</strong></p>
                    <p class="mb-2"><span class="text-slate-500 inline-block w-20">Bisnis</span>: <strong class="text-white">\${bisnis}</strong></p>
                    <p class="mb-2"><span class="text-slate-500 inline-block w-20">Kategori</span>: <strong class="text-white">\${kategori}</strong></p>
                </div>
            \`,
            background: '#0f172a',
            color: '#cbd5e1',
            ...
            if(res.isConfirmed) {
                Swal.fire({
                    title: 'Keluar?',
                    text: 'Anda akan dikembalikan ke halaman depan.',
                    icon: 'warning',
                    showCancelButton: true,
